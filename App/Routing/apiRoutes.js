...
module.exports = function(app) {
    ...
    app.post("/api/friends", function(req, res) {
        ...
        var newFriend = {
            ...
            photo: "",
            difference: 1000
                //difference between answer of user and friend in array- comparing scores
        };
        ...
        var userScore = userResults.scores;
        ...
        for (var i = 0; i < friendsArray.length; i++) {
            ...
            for (var x = 0; x < friendsArray[i].scores[x]; x++) {
                ...
                scoreDifference += Math.abs(parseInt(userScore[x] - parseInt(friendsArray[i].scores[x])));

                //if total difference less current friend then assign that friend to the correct friend
                if (scoreDifference <= newFriend.difference) {
                    //set the friendMatch as the new friend and display?
                    newFriend.name = friendsArray[i].name;
                    newFriend.photo = friendsArray[i].photo;
                    newFriend.difference = scoreDifference;
                }
            }
        }
        //save to database
        friendsArray.push(userResults);

        res.json(newFriend); //for HTML
